...
void exceptionHandler(NSException *exception) {
    ...
    NSArray * tracesArr = [CZCaptureException getBacktraceWithException:exception];
    ...
    [[CZCaptureException shareManager] performSelectorOnMainThread:@selector(handleException:) withObject:[NSException exceptionWithName:exception.name reason:exception.reason userInfo:dic] waitUntilDone:YES];
}
...
/**
 @brief 代理对象
 */
@property (nonatomic, weak) id<CZCaptureExceptionDelegate> delegate;

@end

@implementation CZCaptureException
+(instancetype)shareManager
{
    static CZCaptureException * exception = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        exception = [[CZCaptureException alloc] init];
    });
    return exception;
}
